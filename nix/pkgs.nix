...
#
# `nixpkgs.legacyPackages.<system>` carries no overlays and no `config`, and
...
# So every output imports nixpkgs through this instead.
#
...
import inputs.nixpkgs {
  inherit system;
  # `import ./overlay.nix` rather than `inputs.self.overlays.default`: `pkgs` is
  # what every other file here is evaluated with, so reading it back out of this
  # flake's own outputs is a cycle waiting to happen.
  overlays = [
